...
def count_internal(t: Tree) -> int:
    ...
    if t.children is None:
        ...
        return sum([1 + count_internal(x) for x in t.children])
...
def count_leaves(t: Tree) -> int:
    ...
    if t.children is None:
        ...
        return sum([count_internal(x) for x in t.children])
...
def sum_internal(t: Tree) -> int:
    ...
    if t.children is None:
        ...
        return sum([t.value + sum_internal(x) for x in t.children])
...
def contains_test_passer(t: Tree, test: Callable[[Any], bool]) -> bool:
    ...
    if t.children is None:
        ...
        return any([test(t.value) + contains_test_passer(x, test) for x in t.children])
...
def list_if(t: Tree, p: Callable[[Any], bool]) -> list:
    ...
    True
    """
    if t.children is None and p(t.value):
        return [t.value]
    if t.children is None and not p(t.value):
        return []
    else:
        acc = []
        if p(t.value):
